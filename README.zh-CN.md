...
| Linux | CI 已验证 | 最近一次 GitHub Actions 在 Node.js 20/22/24 下通过；使用前仍需核对本机 Agent 路径。 |
| Windows | CI 已验证 | 最近一次 GitHub Actions 在 Node.js 20/22/24 下通过。Junction 行为仍受本机文件系统策略和权限影响，建议先看 `sync`，再决定是否 `--apply`。 |
